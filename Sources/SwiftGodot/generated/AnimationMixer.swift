...
open class AnimationMixer: Node {
    ...
    public enum AnimationCallbackModeDiscrete: Int64, CaseIterable, CustomDebugStringConvertible {
        ...
        /// 
        /// If a value track has non-numeric type key values, it is internally converted to use .recessive with ``Animation/UpdateMode/updateDiscrete``.
        /// 
    ...
    /// Ordinarily, tracks can be set to ``Animation/UpdateMode/updateDiscrete`` to update infrequently, usually when using nearest interpolation.
    /// 
    /// However, when blending with ``Animation/UpdateMode/updateContinuous`` several results are considered. The ``callbackModeDiscrete`` specify it explicitly. See also ``AnimationMixer/AnimationCallbackModeDiscrete``.
    /// 
    /// To make the blended results look good, it is recommended to set this to .forceContinuous to update every frame during blending. Other values exist for compatibility and they are fine if there is no blending, but not so, may produce artifacts.
    /// 
    ...
    /// If the animation track specified by `name` has an option ``Animation/UpdateMode/updateCapture``, stores current values of the objects indicated by the track path as a cache. If there is already a captured cache, the old cache is discarded.
    /// 
    ...
    /// 
    /// You can specify `transType` as the curve for the interpolation. For better results, it may be appropriate to specify ``Tween/TransitionType/transLinear`` for cases where the first key of the track begins with a non-zero value or where the key value does not change, and ``Tween/TransitionType/transQuad`` for cases where the key value changes linearly.
    /// 
