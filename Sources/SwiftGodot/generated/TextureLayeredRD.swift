...
open class TextureLayeredRD: TextureLayered {
    private static var className = StringName("TextureLayeredRD")
        ...
        withUnsafePointer(to: textureRdRid.content) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(TextureLayeredRD.method_set_texture_rd_rid, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_texture_rd_rid() -> RID {
        if handle == nil { Wrapped.attemptToUseObjectFreedByGodot() }
        let _result: RID = RID ()
        gi.object_method_bind_ptrcall(TextureLayeredRD.method_get_texture_rd_rid, handle, nil, &_result.content)
        return _result
    }
