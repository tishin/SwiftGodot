...
open class VisualShaderNodeParticleRandomness: VisualShaderNode {
    private static var className = StringName("VisualShaderNodeParticleRandomness")
        ...
        withUnsafePointer(to: type.rawValue) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeParticleRandomness.method_set_op_type, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_op_type() -> VisualShaderNodeParticleRandomness.OpType {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeParticleRandomness.method_get_op_type, handle, nil, &_result)
