...
open class SurfaceTool: RefCounted {
    ...
    /// Creates this SurfaceTool from existing vertex arrays such as returned by ``commitToArrays()``, ``Mesh/surfaceGetArrays(surfIdx:)``, ``Mesh/surfaceGetBlendShapeArrays(surfIdx:)``, ``ImporterMesh/getSurfaceArrays(surfaceIdx:)``, and ``ImporterMesh/getSurfaceBlendShapeArrays(surfaceIdx:blendShapeIdx:)``. `primitiveType` controls the type of mesh data, defaulting to ``Mesh/PrimitiveType/primitiveTriangles``.
