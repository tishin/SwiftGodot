...
open class VisualShaderNodeTransformFunc: VisualShaderNode {
    private static var className = StringName("VisualShaderNodeTransformFunc")
        ...
        withUnsafePointer(to: `func`.rawValue) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeTransformFunc.method_set_function, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_function() -> VisualShaderNodeTransformFunc.Function {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeTransformFunc.method_get_function, handle, nil, &_result)
