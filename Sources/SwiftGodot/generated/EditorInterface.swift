...
open class EditorInterface: Object {
    ...
    /// 
    /// > Note: This node is a ``VBoxContainer``, which means that if you add a ``Control`` child to it, you need to set the child's ``Control/sizeFlagsVertical`` to ``Control/SizeFlags/sizeExpandFill`` to make it use the full available space.
    /// 
    /// > Warning: Removing and freeing this node will render a part of the editor useless and may cause a crash.
    /// 
