...
open class VisualShaderNodeTexture3D: VisualShaderNodeSample3D {
        ...
        withUnsafePointer(to: value?.handle) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeTexture3D.method_set_texture, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_texture() -> Texture3D? {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeTexture3D.method_get_texture, handle, nil, &_result)
