...
open class CSGBox3D: CSGPrimitive3D {
        ...
        withUnsafePointer(to: size) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(CSGBox3D.method_set_size, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_size() -> Vector3 {
        ...
        gi.object_method_bind_ptrcall(CSGBox3D.method_get_size, handle, nil, &_result)
        return _result
    }
    ...
    fileprivate final func set_material(_ material: Material?) {
        ...
        withUnsafePointer(to: material?.handle) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(CSGBox3D.method_set_material, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_material() -> Material? {
        ...
        gi.object_method_bind_ptrcall(CSGBox3D.method_get_material, handle, nil, &_result)
