...
open class VisualShaderNodeCubemap: VisualShaderNode {
        ...
        withUnsafePointer(to: value.rawValue) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_set_source, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_source() -> VisualShaderNodeCubemap.Source {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_get_source, handle, nil, &_result)
    ...
    fileprivate final func set_cube_map(_ value: TextureLayered?) {
        ...
        withUnsafePointer(to: value?.handle) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_set_cube_map, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_cube_map() -> TextureLayered? {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_get_cube_map, handle, nil, &_result)
    ...
    fileprivate final func set_texture_type(_ value: VisualShaderNodeCubemap.TextureType) {
        ...
        withUnsafePointer(to: value.rawValue) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_set_texture_type, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_texture_type() -> VisualShaderNodeCubemap.TextureType {
        if handle == nil { Wrapped.attemptToUseObjectFreedByGodot() }
        var _result: Int64 = 0 // to avoid packed enums on the stack
        gi.object_method_bind_ptrcall(VisualShaderNodeCubemap.method_get_texture_type, handle, nil, &_result)
