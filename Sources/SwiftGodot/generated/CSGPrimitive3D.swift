...
open class CSGPrimitive3D: CSGShape3D {
        ...
        withUnsafePointer(to: flipFaces) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(CSGPrimitive3D.method_set_flip_faces, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_flip_faces() -> Bool {
        ...
        gi.object_method_bind_ptrcall(CSGPrimitive3D.method_get_flip_faces, handle, nil, &_result)
        return _result
    }
