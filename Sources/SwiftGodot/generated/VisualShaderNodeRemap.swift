...
open class VisualShaderNodeRemap: VisualShaderNode {
        ...
        withUnsafePointer(to: opType.rawValue) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeRemap.method_set_op_type, UnsafeMutableRawPointer(mutating: handle), pArgs, nil)
                }
    ...
    fileprivate final func get_op_type() -> VisualShaderNodeRemap.OpType {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeRemap.method_get_op_type, UnsafeMutableRawPointer(mutating: handle), nil, &_result)
