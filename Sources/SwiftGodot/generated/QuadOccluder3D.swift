...
open class QuadOccluder3D: Occluder3D {
    private static var className = StringName("QuadOccluder3D")
        ...
        withUnsafePointer(to: size) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(QuadOccluder3D.method_set_size, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_size() -> Vector2 {
        ...
        gi.object_method_bind_ptrcall(QuadOccluder3D.method_get_size, handle, nil, &_result)
        return _result
    }
