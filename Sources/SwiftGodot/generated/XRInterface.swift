...
open class XRInterface: RefCounted {
    ...
    public enum PlayAreaMode: Int64, CaseIterable, CustomDebugStringConvertible {
        ...
        /// Same as .xrPlayAreaRoomscale but origin point is fixed to the center of the physical space. In this mode, system-level recentering may be disabled, requiring the use of ``XRServer/centerOnHmd(rotationMode:keepHeight:)``.
    ...
    /// 
    /// > Note: Changing this after the interface has already been initialized can be jarring for the player, so it's recommended to recenter on the HMD with ``XRServer/centerOnHmd(rotationMode:keepHeight:)`` (if switching to ``XRInterface/PlayAreaMode/xrPlayAreaStage``) or make the switch during a scene change.
    /// 
