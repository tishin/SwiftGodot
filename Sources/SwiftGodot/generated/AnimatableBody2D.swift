...
open class AnimatableBody2D: StaticBody2D {
        ...
        withUnsafePointer(to: enable) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(AnimatableBody2D.method_set_sync_to_physics, handle, pArgs, nil)
                }
    ...
    fileprivate final func is_sync_to_physics_enabled() -> Bool {
        ...
        gi.object_method_bind_ptrcall(AnimatableBody2D.method_is_sync_to_physics_enabled, handle, nil, &_result)
        return _result
    }
