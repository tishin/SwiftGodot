...
open class VisualShaderNodeCurveTexture: VisualShaderNodeResizableBase {
    private static var className = StringName("VisualShaderNodeCurveTexture")
        ...
        withUnsafePointer(to: texture?.handle) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(VisualShaderNodeCurveTexture.method_set_texture, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_texture() -> CurveTexture? {
        ...
        gi.object_method_bind_ptrcall(VisualShaderNodeCurveTexture.method_get_texture, handle, nil, &_result)
