...
open class CapsuleShape2D: Shape2D {
    private static var className = StringName("CapsuleShape2D")
        ...
        withUnsafePointer(to: radius) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(CapsuleShape2D.method_set_radius, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_radius() -> Double {
        ...
        gi.object_method_bind_ptrcall(CapsuleShape2D.method_get_radius, handle, nil, &_result)
        return _result
    }
    ...
    fileprivate final func set_height(_ height: Double) {
        ...
        withUnsafePointer(to: height) { pArg0 in
            withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                    gi.object_method_bind_ptrcall(CapsuleShape2D.method_set_height, handle, pArgs, nil)
                }
    ...
    fileprivate final func get_height() -> Double {
        if handle == nil { Wrapped.attemptToUseObjectFreedByGodot() }
        var _result: Double = 0.0
        gi.object_method_bind_ptrcall(CapsuleShape2D.method_get_height, handle, nil, &_result)
        return _result
    }
